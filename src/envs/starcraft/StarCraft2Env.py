...
from pymarl2.src.envs.starcraft.SkipDataUtils import parseSeqData
from pymarl2.src.chatGPT.QueryGptUtil import query_gpt_in_process, query_gpt_end, query_topology
from pymarl2.src.tapnet import judgeSkip, Hyperparameter
import torch

root_path = './new_results/'
map = game_config.maps[game_config.mapId]
currnet_env = root_path + map
if not os.path.exists(currnet_env):
    os.mkdir(currnet_env)

# LLM
position_agents_path = currnet_env + '/position_agents.txt'
position_enemies_path = currnet_env + '/position_enemies.txt'
actions_agents_path = currnet_env + '/actions_agents.txt'
gpt_topo_results_in_process_path = currnet_env + '/gpt_topo_results_in_process.txt'
gpt_topo_results_end_path = currnet_env + '/gpt_topo_results_end.txt'
gpt_sentence_results_in_process_path = currnet_env + '/gpt_sentence_results_in_process.txt'
gpt_sentence_results_end_path = currnet_env + '/gpt_sentence_results_end.txt'

single_game_position_agents = []
single_game_position_enemies = []
single_game_actions_agents = []
single_game_in_process_topo = []
cur_step = 0


# Rule
unit_distance_path = currnet_env + '/unit_distance.txt'
action_path = currnet_env + '/action.txt'
attack_target_path = currnet_env + '/attack_target.txt'


health_path = currnet_env + '/health.txt'
attack_path = currnet_env + '/attack.txt'
activity_range_path = currnet_env + '/activity_range.txt'

model = judgeSkip.load_tapnet_mode()

def judge_inProcess_skip(single_game_position_agents, single_game_position_enemies):
    # 未达到指定步数
    if (len(single_game_position_agents) < game_config.inProcess_judge_steps
            or len(single_game_position_enemies) < game_config.inProcess_judge_steps):
        return False
    curSeq = parseSeqData(single_game_position_agents, single_game_position_enemies)

    # print(len(curSeq))
    # print(len(curSeq[0]))
    # print(len(game_config.bench_noCrash))
    # print(len(game_config.bench_noCrash[0]))

    tapnet_path = 'C:/Users/wangkr/Desktop/LLM_pymarl/pymarl2/src/tapnet/model/' + Hyperparameter.game_scene + '/tapnet.pkl'
    model.load_state_dict(torch.load(tapnet_path))

    judge_res = judgeSkip.predict_one(model, curSeq)
    if judge_res == 1:
        # 和 bench_noCrash 相似
        return True
    elif judge_res == 0:
...
def parseInProcessTopo(single_game_in_process_topo):
    if len(single_game_in_process_topo) == 0:
        return ''
    in_process_topo = []
    for sublist in single_game_in_process_topo:
        in_process_topo.extend(sublist)
    in_process_topo = remove_adjacent_duplicates(in_process_topo)
    micros = []
    microMap = {'A':'Kiting', 'B':'Scattered Attack', 'C':'Concentrated Attack', 'D':'Alternate Attack'}
    for microInfo in in_process_topo:
        if microInfo not in microMap.keys():
            continue
        micro = microMap[microInfo]
        micros.append(micro)
    ret = ''
    for i in range(len(micros)):
        if i == len(micros) - 1:
            ...
            ret += micros[i] + ' and then'
    return ret

def match(agent_topo, instruction_topo):
    list1, list2 = instruction_topo, agent_topo
    if not list1:
        return True
    idx1 = 0
    len1 = len(list1)
...
def get_LLM_result(single_game_position_agents, single_game_position_enemies, single_game_actions_agents, instruction):
    in_process_topo_analysis = ''
    summary_gpt_res = query_gpt_end(single_game_position_agents, single_game_position_enemies,
                                    single_game_actions_agents, in_process_topo_analysis)
    agent_topo = []
    ways = 'rule'
    if 'unable to provide a response at this time' not in summary_gpt_res:
        if 'No micromanagement is used' not in summary_gpt_res:
            agent_topo = query_topology(summary_gpt_res, ways)
    instruction_topo = query_topology(instruction, ways)
    return match(agent_topo, instruction_topo)


def LLM_process(single_game_position_agents, single_game_position_enemies, single_game_actions_agents, single_game_in_process_topo, terminated_flag, battles_game, skip_flag):
    if skip_flag:
        single_game_position_agents.clear()
        single_game_position_enemies.clear()
        single_game_actions_agents.clear()
        single_game_in_process_topo.clear()

        with open(gpt_topo_results_end_path, 'a') as f:
            f.write('@@@@@@@@@ summary: ' + str(battles_game) + ': \n' + 'tapnet skip' + '\n\n')

        return
    """
    In Process
    """
    seq_len = len(single_game_actions_agents)
    if seq_len % game_config.window_size == 0 and seq_len >= game_config.window_size:
        k = game_config.llm_window_size
        last_k_position_agents = single_game_position_agents[-k:]
        last_k_position_enemies = single_game_position_enemies[-k:]
        last_k_actions_agents = single_game_actions_agents[-k:]
        cur_gpt_res = query_gpt_in_process(last_k_position_agents, last_k_position_enemies, last_k_actions_agents)
        topo = []
        if 'unable to provide a response at this time' not in cur_gpt_res:
            if 'No micromanagement is used' not in cur_gpt_res:
                topo = query_topology(cur_gpt_res, ways='rule')
                if '[]' not in str(topo):
                    single_game_in_process_topo.append(topo)
        with open(gpt_topo_results_in_process_path, 'a') as f:
            f.write(str(battles_game) + ' gpt judge : ' + str(topo) + '\n')
        with open(gpt_sentence_results_in_process_path, 'a') as f:
            f.write(str(battles_game) + ' gpt judge : ' + str(cur_gpt_res) + '\n')

    """  
    End
    """
    if terminated_flag:
        in_process_topo_analysis = parseInProcessTopo(single_game_in_process_topo)
        summary_gpt_res = query_gpt_end(single_game_position_agents, single_game_position_enemies, single_game_actions_agents, in_process_topo_analysis)
        topo = []
        if 'unable to provide a response at this time' not in summary_gpt_res:
            if 'No micromanagement is used' not in summary_gpt_res:
                ways = 'rule'
                topo = query_topology(summary_gpt_res, ways)
        with open(gpt_topo_results_end_path, 'a') as f:
            f.write('@@@@@@@@@ summary: ' + str(battles_game) + ': \n' + str(topo) + '\n\n')
        with open(gpt_sentence_results_end_path, 'a') as f:
            f.write('@@@@@@@@@ summary: ' + str(battles_game) + ': \n' + str(summary_gpt_res) + '\n\n')
        single_game_position_agents.clear()
        single_game_position_enemies.clear()
        single_game_actions_agents.clear()
...
class StarCraft2Env(MultiAgentEnv):
    ...
    def step(self, actions):
        ...
        enemy_state, enemy_health, enemy_shield, respective_health = self.get_enemy_state()
        enemy_state = enemy_state.tolist()
        self.en_this_battle.append(enemy_state)  #en_this_battle记录一局对战的状态序�?
        ally_state, agent_health, shield_sum = self.get_agent_state()
        ally_state = ally_state.tolist()
        self.al_this_battle.append(ally_state)
        ...
        if len(action_path) > 1:
            with open(action_path, 'a') as f:
                f.write('agents respective_attack target are: ' + str(sc_actions) + '\n')
                f.write('-------------' + '\n')

        if terminated:
            with open(action_path, 'a') as f:
                f.write(str(self.battles_game) + ' ****** agent_health_sum - enemy_health_sum: ' + str(minus) + ' ******' + '\n')


        #存储存活的agent各自到最近的敌方单位的距离
        dist_list = []
        for agent_id in range(self.n_agents):
            unit = self.get_unit_by_id(agent_id)
            if unit.health > 0:
                ...
                dist_list.append(min_dis)

        if len(dist_list) > 0:
            with open(unit_distance_path, 'a') as f:
                f.write(str(self.battles_game) + 'min distance are: ' + str(dist_list) + '\n')

        if terminated:
            with open(unit_distance_path, 'a') as f:
                f.write('****** agent_health_sum - enemy_health_sum: ' + str(minus) + ' ******' + '\n')

        # 存储agent和敌人的位置
        position_agent_list = []
        position_enemies_list = []

        for agent_id in range(self.n_agents):
            unit = self.get_unit_by_id(agent_id)
            if unit.health > 0:
                cur_agent_pos = [unit.pos.x, unit.pos.y]
                position_agent_list.append(cur_agent_pos)
                target_items = self.enemies.items()
                for t_id, t_unit in target_items:
                    if t_unit.health > 0:
                        cur_enemies_pos = [t_unit.pos.x, t_unit.pos.y]
                        position_enemies_list.append(cur_enemies_pos)

        if len(position_agent_list) > 0:
            single_game_position_agents.append(position_agent_list)
            with open(position_agents_path, 'a') as f:
                f.write(str(self.battles_game) + ' position of agents are : ' + str(position_agent_list) + '\n')

        if len(position_enemies_list) > 0:
            single_game_position_enemies.append(position_enemies_list)
            with open(position_enemies_path, 'a') as f:
                f.write(str(self.battles_game) + ' position of enemies are : ' + str(position_enemies_list) + '\n')
        ...
        for action in sc_actions:
            ...
            if 'ability_id: 23' in str_action:
                target_unit_tag = str_action.split('target_unit_tag: ')[1]
                target_unit_tag = target_unit_tag.split('unit_tags:')[0]
                target_unit_tag = target_unit_tag.split('\n')[0]
                attack_target.append(int(target_unit_tag))

        output_sum = 0

        if len(attack_target) > 1:
            with open(attack_target_path, 'a') as f:
                f.write('agents respective_attack target are: ' + str(attack_target) + '\n')
        ...
        #   unit_command {
        #     ability_id: 23
        #     target_unit_tag: 4295753730
        #     unit_tags: 4295491587
        #     queue_command: false
        #   }
        # }
        ...
        for action in sc_actions:
            str_action = str(action)
            if 'ability_id: 16' in str_action:
                actions_agents.append('move')
            elif 'ability_id: 23' in str_action:
                target_unit_tag = str_action.split('target_unit_tag: ')[1]
                target_unit_tag = target_unit_tag.split('unit_tags:')[0]
                target_unit_tag = target_unit_tag.split('\n')[0]
                attack_target_id = int(target_unit_tag)
                actions_agents.append('attack' + str(attack_target_id))
            elif 'ability_id: 4' in str_action:
                actions_agents.append('stop')
            elif 'ability_id: 386' in str_action:
                actions_agents.append('heal')

        if len(actions_agents) > 0:
            single_game_actions_agents.append(actions_agents)
            with open(actions_agents_path, 'a') as f:
                f.write(str(self.battles_game) + ' actions of agents are : ' + str(actions_agents) + '\n')
        ...
        InProcess Skip 
        """
        skip_flag = judge_inProcess_skip(single_game_position_agents, single_game_position_enemies)

        code_start_time = game_config.code_start_time
        cur_time = time.time()
        elapsed_time = cur_time - code_start_time  # 计算已经过去的时间
        if elapsed_time > game_config.code_timeout:
            # print("程序执行超时！")
            # print('code_start_time: ', code_start_time)
            # print('cur_time: ', cur_time)

            logging.debug("程序执行超时！")
            logging.debug('code_start_time: ', code_start_time)
            logging.debug('cur_time: ', cur_time)
            sys.exit(0)  # 如果已经超时，那么结束程序
        ...
        LLM Process 
        """
        LLM_process(single_game_position_agents, single_game_position_enemies, single_game_actions_agents, single_game_in_process_topo, terminated, self.battles_game, skip_flag)
    ...
    def get_agent_state(self):
        ...
        return ally_state, health_sum, shield_sum
    ...
    def get_enemy_state(self):
        ...
        for e_id, e_unit in self.enemies.items():
            if e_unit.health > 0:
                ...
                respective_health.append(e_unit.health)
        ...
        return enemy_state, health_sum, shield_sum, respective_health
